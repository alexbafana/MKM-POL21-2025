...
const useAddr = (): `0x${string}` | undefined => {
  ...
  return deployedContracts?.[chainId]?.contracts?.Validation_Committee?.address as `0x${string}` | undefined;
};
...
export default function ValidationCommitteePage() {
  ...
  const [support, setSupport] = useState<number>(1); // 0=Against,1=For,2=Abstain
  const onVote = () => {
    if (!gov) return;
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-1">Validation Committee</h1>
      <p className="opacity-70 mb-6">Chain {chainId} • Contract {gov ? <Address address={gov} /> : "not deployed"}</p>

      {!gov && <div className="alert alert-warning">Validation Committee contract isn’t deployed on this chain.</div>}
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-base-100 rounded-2xl p-5 shadow">
            ...
            <label className="form-control mb-3">
              ...
              <input className="input input-bordered font-mono" placeholder="0x..." value={target} onChange={e => setTarget(e.target.value.trim())} />
            </label>
            <label className="form-control mb-3">
              <span className="label-text">Calldata (hex)</span>
              <input className="input input-bordered font-mono" placeholder="0x…" value={calldataHex} onChange={e => setCalldataHex(e.target.value.trim())} />
            </label>
            <label className="form-control mb-4">
              <span className="label-text">Description</span>
              <input className="input input-bordered" value={desc} onChange={e => setDesc(e.target.value)} />
            </label>
          ...
          <div className="bg-base-100 rounded-2xl p-5 shadow">
            ...
            <label className="form-control mb-3">
              ...
              <input className="input input-bordered" value={proposalId} onChange={e => setProposalId(e.target.value)} />
            </label>
            <label className="form-control mb-4">
              <span className="label-text">Support</span>
