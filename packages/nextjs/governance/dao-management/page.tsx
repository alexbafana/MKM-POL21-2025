  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <button className="btn btn-primary" disabled={!mkmp || isPending} onClick={()=>writeContract({ address: mkmp!, abi: PM_ABI, functionName: "Issue_DID", args: [] })}>Issue DID</button>
        <button className="btn" disabled={!mkmp || isPending} onClick={()=>writeContract({ address: mkmp!, abi: PM_ABI, functionName: "Burn_DID", args: [] })}>Burn DID</button>
        <button className="btn" disabled={!mkmp || isPending} onClick={()=>writeContract({ address: mkmp!, abi: PM_ABI, functionName: "mint_MKMT", args: [] })}>Mint MKMT</button>
        <button className="btn" disabled={!mkmp || isPending} onClick={()=>writeContract({ address: mkmp!, abi: PM_ABI, functionName: "burn_MKMT", args: [] })}>Burn MKMT</button>
        <button className="btn" disabled={!mkmp || isPending} onClick={()=>writeContract({ address: mkmp!, abi: PM_ABI, functionName: "distribute_MKMT", args: [] })}>Distribute MKMT</button>
      </div>
