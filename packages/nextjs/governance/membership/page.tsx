  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <button className="btn btn-primary" disabled={!mkmp || isPending} onClick={()=>writeContract({ address: mkmp!, abi: PM_ABI, functionName: "onboard_ordinary_user", args: [] })}>Onboard Ordinary User</button>
        <button className="btn" disabled={!mkmp || isPending} onClick={()=>writeContract({ address: mkmp!, abi: PM_ABI, functionName: "onboard_institution", args: [] })}>Onboard Institution</button>
        <button className="btn" disabled={!mkmp || isPending} onClick={()=>writeContract({ address: mkmp!, abi: PM_ABI, functionName: "remove_ordinary_member", args: [] })}>Remove Ordinary Member</button>
        <button className="btn" disabled={!mkmp || isPending} onClick={()=>writeContract({ address: mkmp!, abi: PM_ABI, functionName: "remove_institution", args: [] })}>Remove Institution</button>
      </div>
